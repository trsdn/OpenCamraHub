...
/// Draws the "OpenLens isn't running" card exactly once.
///
/// This is a deliberate duplicate of `OpenLensCamera`'s `IdleFrameRenderer`:
/// the app and the camera extension are separate binaries, so there is no
/// shared function to call. `VideoRenderer.setPaused` uses this copy to send
/// the same card while paused, on purpose — a pause should look exactly like
/// the extension's own idle fallback, not like a distinct third state.
///
/// The result is a static buffer that gets re-sent every frame while idle, so
/// the steady-state cost of a pause is a memcpy-free `stream.send`, same as
/// the camera extension's own copy.
enum IdleFrameRenderer {
    ...
    ///
    /// This runs once per pause, so a plain loop is clearer than pulling in an
    ...
    private static func draw(
        ...
        // CoreText keys rather than AppKit's, matching the camera extension's
        // copy: staying free of `NSAttributedString.Key` keeps the two files
        // identical apart from this doc comment.
        let attributes: [CFString: Any] = [
