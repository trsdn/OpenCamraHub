...
Every value is read from its authority (Info.plist, project.yml, git tags), so no
...
def read_license(root: Path = ROOT) -> str:
    with (root / "Sources" / "OpenLens" / "Info.plist").open("rb") as handle:
        return str(plistlib.load(handle)["OCHLicenseIdentifier"])


def read_platform(root: Path = ROOT) -> str:
    manifest = (root / "project.yml").read_text(encoding="utf-8")
    match = re.search(r'^\s*macOS:\s*"?(\d+)(?:\.\d+)*"?\s*$', manifest, re.MULTILINE)
    if not match:
        raise SystemExit("project.yml declares no deploymentTarget macOS version")
